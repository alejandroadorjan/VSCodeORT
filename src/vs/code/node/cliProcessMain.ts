...
class CliMain extends Disposable {
	...
	> {
		...
		// Get machine ID (call for side-effects; ignore returned values)
		try {
			await resolveMachineId(stateService, logService);
		} catch (error) {
			if (error.code !== "ENOENT") {
				logService.error(error);
			}
		}
		await resolveSqmId(stateService, logService);
		await resolveDevDeviceId(stateService, logService);
